 ...
 */
static inline TokenType MatchOperatorType(int32 ch)
{
	switch (ch) {
 ...
 */
void Token::AppendToString(std::string& aBuffer) const
{
  /*switch (mType) {
    case Token_Ident:
    ...
    default:
      NOTREACHED()<<"invalid token type";
      break;
  }*/
}


Scanner::Scanner( const std::string& aBuffer, uint32 aLineNumber )
...
bool Scanner::Next( Token& aToken, bool aSkipWS )
{
	int32 ch;
	...
	// Match operators: ~= |= ^= $= *=
	TokenType opType = MatchOperatorType(ch);
...
bool Scanner::NextURL( Token& aToken )
{
	SkipWhitespace();
...
void Scanner::StopRecording()
{
	DCHECK(mRecording)<<"haven't started recording";
	mRecording = false;
}

void Scanner::StopRecording( std::string& aBuffer )
{
	DCHECK(mRecording)<<"haven't started recording";
	mRecording = false;
...
void Scanner::Advance( uint32 n /*= 1*/ )
{
 ...
 */
bool Scanner::GatherEscape( std::string& aOutput, bool aInString )
{
	int32 ch = Peek(1);
	if (ch < 0) {
...
bool Scanner::GatherText( uint8 aClass, std::string& aText )
{
...
bool Scanner::ScanIdent( Token& aToken )
{
	...
	if (LowerCaseEqualsASCII(aToken.mIdent, "url")){
		NextURL(aToken);
	}
	return true;
}
 ...
 */
bool Scanner::ScanAtKeyword( Token& aToken )
{
 ...
 */
bool Scanner::ScanHash( Token& aToken )
{
...
bool Scanner::ScanNumber( Token& aToken )
{
	int32 c = Peek();
 ...
 */
bool Scanner::ScanString( Token& aToken )
{
	int32 aStop = Peek();
 ...
 * form.
 */
bool Scanner::ScanURange( Token& aResult )
{
	int32 intro1 = Peek();
	int32 intro2 = Peek(1);
	int32 ch = Peek(2);
