...
namespace css
{
	...
	public:
		...
		Scanner(const std::string& aBuffer, uint32 aLineNumber);
		~Scanner();
		...
		bool Next(Token& aTokenResult, bool aSkipWS);
		...
		// parser.
		bool NextURL(Token& aTokenResult);
	...
	protected:
		...
		bool ScanIdent(Token& aResult);
		bool ScanAtKeyword(Token& aResult);
		bool ScanHash(Token& aResult);
		bool ScanNumber(Token& aResult);
		bool ScanString(Token& aResult);
		bool ScanURange(Token& aResult);
